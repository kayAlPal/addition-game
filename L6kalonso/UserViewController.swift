    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if saveButton === sender {
            ...
            if genderSegmentedControl.selectedSegmentIndex == 0 {
                detailItem = User(newPic: theImage, newName: name, girl: false, highScore: levelLabel.text)
            } else {
                detailItem = User(newPic: theImage, newName: name, girl: true, highScore: levelLabel.text)
            }
